...
create policy "Can read workspace" on workspaces
  for select to authenticated
    using (current_user_is_member_of_organization(organization_id));

create policy "Can delete workspace" on workspaces
  for delete to authenticated
    using (current_user_is_member_of_organization(organization_id));

create policy "Can update workspace" on workspaces
  for update to authenticated
    using (current_user_is_member_of_organization(organization_id))
    with check (current_user_is_member_of_organization(organization_id));

create policy "Can insert workspace" on workspaces
...
select
  (isnt_empty($$
      select
        * from workspaces
        where
          workspaces.name = 'Test Workspace' $$, 'can read a workspace if it belongs to the same organization'));
...
select
  (is_empty($$
      select
        * from workspaces
        where
          workspaces.name = 'Test Workspace' $$, 'cannot read a workspace if it does not belong to the same organization'));
...
select
  (isnt_empty($$
      select
        * from workspaces
        where
          workspaces.name = 'Test Workspace 2' $$, 'verify update worked'));

select
  tests.authenticate_as('user-2');

select
  (lives_ok($$ delete from workspaces
...
select
  (isnt_empty($$
      select
        * from workspaces
        where
          workspaces.name = 'Test Workspace 2' $$, 'verify delete did not work'));
...
select
  (is_empty($$
      select
        * from workspaces
        where
          workspaces.name = 'Test Workspace 2' $$, 'verify delete did work'));

select
  tests.authenticate_as('user-2');

select
  throws_ok($$ insert into workspaces(
...
select
  lives_ok($$ insert into workspaces(
      name, organization_id)
    values (
      'Test Workspace 2', makerkit.get_organization_id(
        'Test Organization'));
