  ...
  return (
    ...
    >
      <ContentEditable
        ...
        className={clsx(
          'text-center text-lg font-semibold leading-tight tracking-wide',
      ...
      {query.getOptions().enabled ? (
        ...
        <div className="mt-14 inline-flex w-full flex-nowrap overflow-hidden">
          <ul className="flex animate-infinite-scroll-horizontal items-center justify-center md:justify-start">
            {logos.map((logo) => (
              <li key={logo.id} className="mx-8">
                <Image
                  priority
          ...
          <ul
            className="flex animate-infinite-scroll-horizontal items-center justify-center md:justify-start"
            aria-hidden="true"
          >
            {logos.map((logo) => (
              <li key={logo.id} className="mx-8">
...
function SidebarDraggableItem({ hasActiveSub }: { hasActiveSub: boolean }) {
  return (
    <SidebarItem
      ...
      image="https://ablcaocvmgtcodafwvoe.supabase.co/storage/v1/object/public/components/logo_6-1699792628850.webp"
      name="Logo 6"
      Component={Logo6}
    />
  );
...
function prepForPageExport(
  ...
  importStatements.push(`import Logo6 from './components/Logo6';`);
  componentContent.push('<Logo6 />');
  return zip.file('components/Logo6.jsx', content);
}
