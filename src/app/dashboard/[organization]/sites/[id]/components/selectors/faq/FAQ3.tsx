...
function SidebarDraggableItem({ hasActiveSub }: { hasActiveSub: boolean }) {
  return (
    <SidebarItem
      ...
      image="https://ablcaocvmgtcodafwvoe.supabase.co/storage/v1/object/public/components/faq_3-1699970900176.webp"
      name="FAQ 3"
      Component={FAQ3}
    />
  );
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <dl className="mt-24 space-y-6">
          {faqs.map((faq) => (
            <div key={faq.id} className="pt-6">
              <dt>
                <div className="flex w-full items-start justify-between text-left ${colors[textColorKey].question}">
...
function prepForPageExport(
  ...
  importStatements.push(`import FAQ3 from './components/FAQ3';`);
  componentContent.push('<FAQ3 />');
  return zip.file('components/FAQ3.jsx', content);
}
