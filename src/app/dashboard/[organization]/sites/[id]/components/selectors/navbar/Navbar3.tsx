...
function SidebarDraggableItem({ hasActiveSub }: { hasActiveSub: boolean }) {
  return (
    <SidebarItem
      ...
      image="https://ablcaocvmgtcodafwvoe.supabase.co/storage/v1/object/public/components/navbar_3-1699474947122.webp"
      name="Navbar 3"
      Component={Navbar3}
    />
  );
...
const navigation = [
  { name: '${removeHtmlTags(navigation[0].name)}', href: '#' },
  { name: '${removeHtmlTags(navigation[1].name)}', href: '#' },
  { name: '${removeHtmlTags(navigation[2].name)}', href: '#' },
];
              ...
              <div className="space-y-2 py-6">
                {navigation.map((item) => (
                  <Link
                    key={item.name}
                    href={item.href}
                    className="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 ${colors[colorKey].mobileHover} ${colors[textColorKey].links}"
                  >
                    {item.name}
                  </Link>
                ))}
...
function prepForPageExport(
  ...
  importStatements.push(`import Navbar3 from './components/Navbar3';`);
  componentContent.push('<Navbar3 />');
  return zip.file('components/Navbar3.jsx', content);
}
