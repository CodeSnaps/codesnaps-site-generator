...
function SidebarDraggableItem({ hasActiveSub }: { hasActiveSub: boolean }) {
  return (
    <SidebarItem
      ...
      image="https://ablcaocvmgtcodafwvoe.supabase.co/storage/v1/object/public/components/team_6-1700175213616.webp"
      name="Team 6"
      Component={Team6}
    />
  );
...
function generateComponentString({
    ...
    (member) => `
  {
    ...
    socials: ${JSON.stringify(team[0].socials)},
  }`,
  );
...
export default function Team() {
  return (
    <div className="${clsx(
        ...
        <dl className="grid grid-cols-1 gap-14 lg:grid-cols-2 xl:gap-20">
          {team.map((member) => (
            <div key={member.id} className="mx-auto grid grid-cols-5 gap-8">
              ...
              <div className="col-span-5 my-auto sm:col-span-3 xl:col-span-2">
                <dt className="flex flex-col items-start gap-y-3">
                  <h3 className="text-lg font-semibold leading-tight ${colors[textColorKey].memberName}">
                    {member.name}
                  </h3>
                  <p className="text-base font-medium ${colors[textColorKey].description}">
                    {member.title}
                  </p>
                </dt>

                <dd className="flex flex-auto flex-col space-y-4 text-base leading-7">
                  <p className="mt-2 max-w-sm text-base ${colors[textColorKey].description}">
                    {member.description}
                  </p>
      ...
      <div className="mx-auto mt-14 lg:mt-24">
        <h3 className="text-2xl font-semibold leading-tight ${colors[textColorKey].heading}">
          ${removeHtmlTags(secondHeading)}
        </h3>

        <p className="mt-6 max-w-xl text-lg${colors[textColorKey].description}">
...
function prepForPageExport(
  ...
  importStatements.push(`import Team6 from './components/Team6';`);
  componentContent.push('<Team6 />');
  return zip.file('components/Team6.jsx', content);
}

Team6.craft = {
  props: {
    tagline: 'Meet Our Team',
    heading: 'The People Behind the Scenes',
    description: 'A brief introduction to our team members.',
    secondHeading: 'We are hiring!',
    secondDescription: 'We’re always looking for talented individuals.',
    cta: 'Apply Now',
    team: team,
    paddingArray: ['px-4', 'sm:px-6', 'lg:px-8'],
    marginArray: ['mx-auto', 'mt-24', 'sm:mt-32', 'lg:mt-40'],
