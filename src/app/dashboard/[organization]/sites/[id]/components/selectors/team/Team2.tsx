...
function SidebarDraggableItem({ hasActiveSub }: { hasActiveSub: boolean }) {
  return (
    <SidebarItem
      ...
      image="https://ablcaocvmgtcodafwvoe.supabase.co/storage/v1/object/public/components/team_2-1699789696636.webp"
      name="Team 2"
      Component={Team2}
    />
  );
...
export default function Team() {
  return (
    <div className="${clsx(
      ...
      <div className="mt-14 grid grid-cols-1 gap-x-10 gap-y-14 sm:grid-cols-2 lg:mt-24 lg:grid-cols-3">
        {team.map((member) => (
          <div key={member.id} className="flex flex-col space-y-6">
            ...
            <p className="mx-auto mt-2 max-w-md text-base ${colors[textColorKey].memberDescription}">
              {member.description}
            </p>
      ...
      <div className="mx-auto mt-14 text-center lg:mt-24">
        <h3 className="text-2xl font-semibold leading-tight ${colors[textColorKey].tagline}">
...
function prepForPageExport(
  ...
  importStatements.push(`import Team2 from './components/Team2';`);
  componentContent.push('<Team2 />');
  return zip.file('components/Team2.jsx', content);
}
