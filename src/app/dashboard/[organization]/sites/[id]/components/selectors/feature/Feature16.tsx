                  ...
                  <dd className="flex flex-auto flex-col text-base leading-7">
                    <ContentEditable
                      ...
                      onChange={(e) =>
                        setProp(
                          (props: any) =>
                            (props.features[feature.id - 1].description =
                              e.target.value),
                        )
                      }
                      tagName="p"
...
function SidebarDraggableItem({ hasActiveSub }: { hasActiveSub: boolean }) {
  return (
    <SidebarItem
      ...
      image="https://ablcaocvmgtcodafwvoe.supabase.co/storage/v1/object/public/components/feature_16-1699974040701.webp"
      name="Feature 16"
      Component={Feature16}
    />
  );
          ...
          <dl className="flex max-w-xl flex-col gap-8">
            {features.map((feature) => (
              <div key={feature.id} className="flex gap-8">
                ...
                <div className="flex flex-col gap-2">
                  <dt className="text-xl font-semibold leading-7 ${colors[textColorKey].heading}">
                    {feature.name}
                  </dt>
...
function prepForPageExport(
  ...
  importStatements.push(`import Feature16 from './components/Feature16';`);
  componentContent.push('<Feature16 />');
  return zip.file('components/Feature16.jsx', content);
}
