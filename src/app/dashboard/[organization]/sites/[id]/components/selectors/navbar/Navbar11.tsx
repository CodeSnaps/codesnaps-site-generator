  ...
  return (
    <div ref={(ref) => connect(drag(ref as HTMLElement))} className="relative">
      <header className="absolute inset-x-0 top-0 z-50">
        <nav
          className={clsx(
            'flex flex-col divide-y px-6 lg:px-0',
          ...
          <div className="mx-auto flex w-full max-w-7xl items-center justify-between py-6 lg:px-8">
            <div className="flex lg:flex-1">
              <Link href="#" className="-m-1.5 p-1.5">
                ...
                <Logo
                  className={clsx('h-8 w-auto z-0', colors[colorKey].logo)}
                />
              </Link>
            </div>
            ...
            <div className="hidden lg:flex lg:flex-1 lg:justify-end lg:space-x-4">
              <ContentEditable
                ...
                className={clsx(
                  'flex rounded-md px-4 py-2.5 text-sm font-semibold shadow-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2',
...
function SidebarDraggableItem({ hasActiveSub }: { hasActiveSub: boolean }) {
  return (
    <SidebarItem
      ...
      image="https://ablcaocvmgtcodafwvoe.supabase.co/storage/v1/object/public/components/navbar_11-1701386973510.webp"
      name="Navbar 11"
      Component={Navbar11}
    />
  );
...
export default function Navbar() {
  ...
  return (
    <header className="absolute inset-x-0 top-0 z-50">
      <nav
        className="flex flex-col divide-y px-6 lg:px-0 ${colors[colorKey].header}"
        aria-label="Global"
      >
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between py-6 lg:px-8">
          <div className="flex lg:flex-1">
        ...
        <Dialog.Panel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto px-6 py-6 sm:max-w-sm sm:ring-1 ${colors[colorKey].dialogPanel}">
          ...
          <div className="mt-6 flow-root">
            <div className="-my-6 divide-y ${colors[colorKey].mobileDivide}">
              <div className="space-y-2 py-6">
                {navigation.map((item) => (
                  <Link
                    key={item.name}
                    href={item.href}
                    className="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 ${colors[textColorKey].mobileDropdownLinks} ${colors[colorKey].mobileHover}"
                    {item.name}
                  </Link>
                ))}
              </div>
              ...
              <Link
                href="#"
                className="flex rounded-md px-4 py-2.5 text-sm font-semibold shadow-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 ${colors[colorKey].primaryCta}"
              >
                ${removeHtmlTags(primaryCta)}
              </Link>
            </div>
...
function prepForPageExport(
  ...
  const { navigation, primaryCta, color, textColor } = node.props;
  ...
  importStatements.push(`import Navbar11 from './components/Navbar11';`);
  componentContent.push('<Navbar11 />');
  return zip.file('components/Navbar11.jsx', content);
}
