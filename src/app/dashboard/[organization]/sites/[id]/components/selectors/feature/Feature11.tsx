...
export const Feature11 = ({
  ...
  heading?: string;
  description?: string;
  featureOneHeading?: string;
  featureOneDescription?: string;
  featureTwoHeading?: string;
      ...
      <div className="grid grid-cols-1 items-center gap-14 lg:grid-cols-2">
        <div className="mx-auto flex max-w-3xl flex-col gap-8">
          ...
          <div className="grid gap-8 md:grid-cols-2">
            <div>
              <ContentEditable
                html={featureOneHeading}
                onChange={(e) =>
                  setProp(
                    (props: { featureOneHeading: string }) =>
                      (props.featureOneHeading = e.target.value),
                  )
                }
                tagName="h3"
                disabled={query.getOptions().enabled ? false : true}
              ...
              <ContentEditable
                html={featureOneDescription}
                onChange={(e) =>
                  setProp(
                    (props: { featureOneDescription: string }) =>
                      (props.featureOneDescription = e.target.value),
                  )
                }
                tagName="p"
                disabled={query.getOptions().enabled ? false : true}
            ...
            <div>
              <ContentEditable
                html={featureTwoHeading}
                onChange={(e) =>
                  setProp(
                    (props: { featureTwoHeading: string }) =>
                      (props.featureTwoHeading = e.target.value),
                  )
                }
                tagName="h3"
                disabled={query.getOptions().enabled ? false : true}
              ...
              <ContentEditable
                html={featureTwoDescription}
                onChange={(e) =>
                  setProp(
                    (props: { featureTwoDescription: string }) =>
                      (props.featureTwoDescription = e.target.value),
                  )
                }
                tagName="p"
                disabled={query.getOptions().enabled ? false : true}
...
function SidebarDraggableItem({ hasActiveSub }: { hasActiveSub: boolean }) {
  return (
    <SidebarItem
      ...
      image="https://ablcaocvmgtcodafwvoe.supabase.co/storage/v1/object/public/components/feature_11-1699973112463.webp"
      name="Feature 11"
      Component={Feature11}
    />
  );
...
function ToolbarSettings() {
  const {
    ...
    featureOneHeading: node.data.props.featureOneHeading,
    featureOneDescription: node.data.props.featureOneDescription,
    featureTwoHeading: node.data.props.featureTwoHeading,
    featureTwoDescription: node.data.props.featureTwoDescription,
      ...
      <div className="grid grid-cols-1 items-center gap-14 lg:grid-cols-2">
        <div className="mx-auto flex max-w-3xl flex-col gap-8">
          ...
          <p className="text-lg ${colors[textColorKey].description}">
           ...
           <div className="grid gap-8 md:grid-cols-2">
            <div>
              <h3 className="text-3xl font-semibold xl:text-4xl ${colors[textColorKey].heading}">
                ${removeHtmlTags(featureOneHeading)}
              </h3>
              <p className="mt-2 ${colors[textColorKey].description}">
                ${removeHtmlTags(featureOneDescription)}
              </p>
            </div>

            <div>
              <h3 className="text-3xl font-semibold xl:text-4xl ${colors[textColorKey].heading}">
                ${removeHtmlTags(featureTwoHeading)}
              </h3>
              <p className="mt-2 ${colors[textColorKey].description}">
                ${removeHtmlTags(featureTwoDescription)}
              </p>
            </div>
          </div>
...
function prepForPageExport(
  ...
  importStatements.push(`import Feature11 from './components/Feature11';`);
  componentContent.push('<Feature11 />');
  return zip.file('components/Feature11.jsx', content);
}
...
Feature11.craft = {
  props: {
    ...
    featureOneHeading: '5 GB',
    featureOneDescription:
      'Suspendisse varius enim in eros elementum tristique.',
    featureTwoHeading: '20 GB',
    featureTwoDescription:
      'Suspendisse varius enim in eros elementum tristique.',
