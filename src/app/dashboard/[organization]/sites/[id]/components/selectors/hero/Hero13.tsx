    ...
    >
      {query.getOptions().enabled ? (
        <div
          style={{
            backgroundImage:
              'url("https://dummyimage.com/1920x1200/f5f5f5/171717")',
            backgroundSize: 'cover',
            backgroundPosition: 'center',
          }}
        >
          <div
            className="absolute inset-0 hidden bg-gradient-to-r from-black/90 to-transparent lg:block"
            aria-hidden="true"
          />
        ...
        <>
          <Image
            ...
            fill={true}
            alt="hero image"
            className="absolute -z-5 h-full max-h-[90vh] w-full object-cover brightness-[0.3] lg:brightness-100"
            aria-hidden="true"
          />
          ...
          <div className="relative z-10 mx-auto mb-24 grid max-w-7xl translate-y-2/4 gap-2 px-4 sm:grid-cols-3 sm:px-6 lg:gap-10 lg:px-8">
            {items.map((item) => (
              <div
                key={item.id}
                className={clsx(
                  'mx-auto max-w-sm rounded-xl px-10 py-8 shadow-md',
                  colors[colorKey].card,
                )}
              >
                <Icon className={clsx('h-8 w-8', colors[colorKey].icon)} />
...
function SidebarDraggableItem({ hasActiveSub }: { hasActiveSub: boolean }) {
  return (
    <SidebarItem
      ...
      image="https://ablcaocvmgtcodafwvoe.supabase.co/storage/v1/object/public/components/hero_13-1700432167908.webp"
      name="Hero 13"
      Component={Hero13}
    />
  );
...
function prepForPageExport(
  ...
  importStatements.push(`import Hero13 from './components/Hero13';`);
  componentContent.push('<Hero13 />');
  return zip.file('components/Hero13.jsx', content);
}
